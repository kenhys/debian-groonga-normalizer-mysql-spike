...
UPSTREAM=$HOME/work/groonga-normalizer-mysql/groonga-normalizer-mysql/packages/debian
BUILDDIR=/var/cache/pbuilder/build
BUILDRESULTDIR=/var/cache/pbuilder/unstable-amd64/result
LOCALPOOLDIR=$HOME/work/debian/groonga-armhf-repository/repositories/armhf/debian/pool/unstable/main/g/groonga-normalizer-mysql
...
{
    VERSION=`echo $1 | sed -e 's/.tar.gz//' | sed -e s'/.\///' | sed -e 's/groonga-normalizer-mysql-//'`
    echo $VERSION
}
...
case $1 in
	...
	if [ ! -f "${PACKAGE}_${VERSION}.orig.tar.gz" ]; then
	    cp -p ${PACKAGE}-${VERSION}.tar.gz ${PACKAGE}_${VERSION}.orig.tar.gz
	fi
	if [ ! -d "$TARGET" ]; then
